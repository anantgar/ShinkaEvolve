"""External verifier for AlphaEvolve's (n,d)=(14,3) distance-ratio task.
...
def assess(value: Any) -> tuple[float, dict[str, float]]:
    ...
    if points.shape != (14, 3) or not np.all(np.isfinite(points)):
        raise ValueError("construct() must return a finite array with shape (14, 3)")
    delta = points[:, None, :] - points[None, :, :]
    squared = np.einsum("ijk,ijk->ij", delta, delta)[np.triu_indices(14, 1)]
