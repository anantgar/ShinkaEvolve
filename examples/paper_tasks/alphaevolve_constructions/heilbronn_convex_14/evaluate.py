"""External verifier for AlphaEvolve's convex-body Heilbronn n=14 task.
...
def assess(value: Any) -> tuple[float, dict[str, float]]:
    ...
    if points.shape != (14, 2) or not np.all(np.isfinite(points)):
        raise ValueError("construct() must return a finite array with shape (14, 2)")
    hull = _hull(points)
    hull_area = (
        abs(
