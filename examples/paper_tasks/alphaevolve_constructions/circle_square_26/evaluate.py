"""External verifier for AlphaEvolve's 26-circle variable-radius task.
...
def assess(value: Any) -> tuple[float, dict[str, float]]:
    ...
    if circles.shape != (26, 3) or not np.all(np.isfinite(circles)):
        raise ValueError(
            "construct() must return finite (x,y,r) rows with shape (26, 3)"
        )
