"""External evaluator for AlphaEvolve's n=95 minimum-overlap task.

The feasibility conditions and correlation objective follow section B.5 of
...
def assess(value: Any) -> tuple[float, dict[str, float]]:
    ...
    if sequence.shape != (95,) or not np.all(np.isfinite(sequence)):
        raise ValueError("construct() must return 95 finite values")
    if np.any(sequence < 0.0) or np.any(sequence > 1.0):
        raise ValueError("all entries must lie in [0,1]")
    if float(np.sum(sequence)) != 47.5:
        raise ValueError("the sequence sum must equal n/2 exactly")
    correlation = np.correlate(sequence, 1.0 - sequence, mode="full")
    bound = float(2.0 * np.max(correlation) / 95)
    return -bound, {"upper_bound_c5": bound}
