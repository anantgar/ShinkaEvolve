"""External evaluator for AlphaEvolve's C1 autocorrelation bound.

The objective follows section B.1 of AlphaEvolve's released
...
def assess(value: Any) -> tuple[float, dict[str, float]]:
    ...
    if sequence.shape != (600,) or not np.all(np.isfinite(sequence)):
        raise ValueError("construct() must return 600 finite values")
    ...
    bound = float(2 * len(sequence) * np.max(convolution) / np.sum(sequence) ** 2)
    return -bound, {"upper_bound_c1": bound}
